...
def launch_setup(context, *args, **kwargs):
    ...
    if (camera_model_val == 'virtual' and float(custom_baseline_val) <= 0):
        return [
            LogInfo(msg="Please set a positive value for the 'custom_baseline' argument when "
                        "using a 'virtual' Stereo Camera with two ZED X One devices."),
        ]
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                ...
                description='The name of the camera. It can be different from the camera '
            ...
            DeclareLaunchArgument(
                'container_name',
                default_value='',
                description='The name of the container to be used to load the ZED component. '
                            'If empty (default) a new container will be created.'),
            DeclareLaunchArgument(
                'namespace',
                default_value='',
                description='The namespace of the node. If empty (default) the camera name '
                            'is used.'),
            DeclareLaunchArgument(
                'node_name',
                default_value='zed_node',
                description='The name of the zed_wrapper node. All the topic will have the '
                            'same prefix: `/<camera_name>/<node_name>/`. If a namespace is '
                            'specified, the node name is replaced by the camera name.'),
            DeclareLaunchArgument(
                'ros_params_override_path',
                default_value='',
                description='The path to an additional parameters file to override the '
                            'default values.'),
            DeclareLaunchArgument(
                'ffmpeg_config_path',
            ...
            DeclareLaunchArgument(
                'serial_number',
                default_value='0',
                description='The serial number of the camera to be opened. It is mandatory to '
                            'use this parameter or camera ID in multi-camera rigs to distinguish '
                            'between different cameras. Use `ZED_Explorer -a` to retrieve the '
                            'serial number of all the connected cameras.'),
            DeclareLaunchArgument(
                'camera_id',
                default_value='-1',
                description='The ID of the camera to be opened. It is mandatory to use this '
                            'parameter or serial number in multi-camera rigs to distinguish '
                            'between different cameras. Use `ZED_Explorer -a` to retrieve the '
            ...
            DeclareLaunchArgument(
                'publish_map_tf',
                default_value='true',
                description='Enable publication of the `map -> odom` TF. Note: Ignored if '
                            '`publish_tf` is False.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'publish_imu_tf',
                default_value='false',
                description='Enable publication of the IMU TF. Note: Ignored if '
                            '`publish_tf` is False.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'xacro_path',
            ...
            DeclareLaunchArgument(
                'enable_gnss',
                default_value='false',
                description='Enable GNSS fusion to fix positional tracking pose with GNSS data '
                            'from messages of type `sensor_msgs::msg::NavSatFix`. The fix topic '
                            'can be customized in `common_stereo.yaml`.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'gnss_antenna_offset',
                default_value='[]',
                description='Position of the GNSS antenna with respect to the mounting point of '
                            'the ZED camera. Format: [x,y,z]'),
            DeclareLaunchArgument(
                'use_sim_time',
                default_value='false',
                description='If set to `true` the node will wait for messages on the `/clock` '
                            'topic to start and will use this information as the timestamp '
                            'reference.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'sim_mode',
                default_value='false',
                description='Enable simulation mode. Set `sim_address` and '
                            '`sim_port` to configure the simulator input.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'sim_address',
                default_value='127.0.0.1',
                description='The connection address of the simulation server. '
                            'See the documentation of the supported '
                            'simulation plugins for more information.'),
            DeclareLaunchArgument(
                'sim_port',
                default_value='30000',
            ...
            DeclareLaunchArgument(
                'custom_baseline',
                default_value='0.0',
                description='Distance between the center of ZED X One cameras in a custom stereo '
                            'rig.'),
